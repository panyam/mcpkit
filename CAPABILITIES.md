...
- mcp-tasks-v2-types: core/task_v2.go — SEP-2663 wire types: CreateTaskResult ({result_type, task}), DetailedTask discriminated union with WorkingTask/InputRequiredTask/CompletedTask/FailedTask/CancelledTask aliases, GetTaskResult alias, TaskInfoV2 (ttlSeconds, pollIntervalMilliseconds, no parentTaskId), UpdateTaskRequest, empty-ack UpdateTaskResult/CancelTaskResult, TaskError (JSON-RPC shape). result_type is snake_case per SEP-2322 conformance.
