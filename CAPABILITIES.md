...
- mcp-tasks-experimental: experimental/ext/tasks — MCP Tasks protocol (spec 2025-11-25). Async tool execution with lifecycle tracking. tasks/get, tasks/result, tasks/list, tasks/cancel. Middleware intercepts tools/call with task hints. InMemoryTaskStore with channel-based WaitForResult/WaitForUpdate. 61 tests.
...
- mcp-tasks-library-experimental: experimental/ext/tasks — EXPERIMENTAL library implementing MCP Tasks protocol (spec 2025-11-25, wire-format parity with TS SDK). tasks.Register(Config) installs middleware + method handlers. Middleware intercepts tools/call with params.task hint (spec-correct location), runs tool async, returns CreateTaskResult. Method handlers: tasks/get (flat Result&Task), tasks/result (original ToolResult + _meta related-task), tasks/list (cursor pagination), tasks/cancel (flat). InMemoryTaskStore with sync.Cond WaitForResult. Client helpers: GetTask, GetTaskPayload, ListTasks, CancelTask, ToolCallAsTask. Nested TasksCap for capability negotiation. Per-tool control via ToolDef.Execution.TaskSupport (required/optional/forbidden, absent=forbidden). 37 tests (15 store + 15 integration + 7 spec-compliance).
