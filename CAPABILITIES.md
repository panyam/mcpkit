...
- mcp-tasks-v2-types: core/task_v2.go — SEP-2663 wire types: CreateTaskResult (flat `Result & Task` — embeds TaskInfoV2 so resultType/taskId/status/ttlSeconds/... all sit at the top level, no nested `task` wrapper), DetailedTask discriminated union with WorkingTask/InputRequiredTask/CompletedTask/FailedTask/CancelledTask aliases, GetTaskResult alias, TaskInfoV2 (ttlSeconds, pollIntervalMilliseconds, no parentTaskId), UpdateTaskRequest, empty-ack UpdateTaskResult/CancelTaskResult, TaskError (JSON-RPC shape). resultType is camelCase like the rest of the MCP wire (Luca confirmed camelCase is the SEP-2322 spec standard).
...
- mcp-tasks-v2-server: server/tasks_v2.go — server.RegisterTasks(TasksConfig) is the canonical SEP-2663 entry point (was RegisterTasksV2). Internals: taskV2Middleware gates on extension (session OR SEP-2575 per-request _meta), gateOnTasksExtension wraps tasks/get/update/cancel handlers, toTaskInfoV2 boundary helper converts internal TaskInfo (ms) to wire TaskInfoV2 (seconds). tasks/cancel returns empty {} ack but still notifies DetailedTask via SSE.
...
- mcp-tasks-v2-client: client/tasks.go — canonical SEP-2663 helpers: ToolCall (returns polymorphic *ToolCallResult with Sync or Task; IsTask() helper for branch readability), GetTask (TaskOptions{RequestState}), UpdateTask (typed core.UpdateTaskRequest, client-side TaskID guard), CancelTask (TaskOptions), WaitForTask (WaitOptions{PollInterval, RequestState}; honors server's PollIntervalMilliseconds with 1s floor + 30s ceiling; threads requestState echo automatically). client.WithTasksExtension() option.
- mcp-tasks-v2-hybrid: server/tasks_hybrid.go — RegisterTasksHybrid(TasksHybridConfig) hosts both v1 and v2 surfaces on one server for rolling client upgrades. Advertises both capabilities.tasks AND capabilities.extensions. Both middlewares stack (v2 first; each gates internally so they don't conflict). hybridDispatch routes tasks/get / tasks/cancel by negotiated capability (extension → v2; else → v1). tasks/update is v2-only (gateOnTasksExtension); tasks/result + tasks/list are v1-only (rejectIfV2 wrapper returns -32601 to v2 clients so they don't see incompatible wire shapes). v2 wins ties when a client declared both.
- mcp-tasks-v2-conformance: conformance/tasks-v2/ — 33 scenarios testing the full SEP-2663 surface. Brand-neutral (assertions track spec text, not mcpkit specifics). Covers extension advertisement + negotiation gating, polymorphic tools/call, DetailedTask with inlined result/error/inputRequests, ttlSeconds + pollIntervalMilliseconds wire renames, ack-only cancel, tasks/update MRTR loop including partial inputResponses fulfillment (v2-29), CreateTaskResult forbidden-field + ISO-8601 timestamp validation, tasks/get with unknown taskId returns -32602 (v2-30, mirrors v2-08), legacy `task` param tolerated/ignored (v2-31), SEP-2575 per-request opt-in, related-task _meta absence on inlined results, status notifications, strong-consistency immediate tasks/get (v2-27), stale-but-valid requestState tolerance (v2-28), SEP-2243 server tolerates Mcp-Method/Mcp-Name request headers — body authoritative — (v2-24/24b/24c). Mcp-Name *response* header (mcpkit-specific echo for proxies) is covered by Go tests `TestV2_McpName*` / `TestV2_McpMethod*` instead. Raw fetch helpers accept opts.headers for arbitrary request-header injection. Backed by fixtures in examples/tasks-v2 (greet, slow_compute, failing_job, protocol_error_job, confirm_delete, multi_input, external_job). Self-contained make target (build + serve + test + teardown). Migration guide: docs/TASKS_V2_MIGRATION.md.
- mcp-mrtr-ephemeral: SEP-2322 ephemeral MRTR (Multi Round-Trip Requests) — server returns IncompleteResult{resultType:"incomplete", inputRequests, requestState}, client retries SAME tools/call with inputResponses + echoed requestState. Stateless: dispatch decodes prior answers from requestState (core.MRTRRoundState), merges with current round's inputResponses, surfaces unified map to handler. core.IncompleteResult type + Sign/VerifyMRTRState (HMAC-SHA256, 24h default TTL) + Encode/DecodeMRTRStatePlaintext (no-key fallback). server.WithRequestStateSigning(key, ttl) — shared by MRTR and SEP-2663 Tasks (TasksConfig.RequestStateKey overrides per-RegisterTasks). Tool handler API: ctx.RequestInput(reqs) sentinel, ctx.InputResponse(key) / HasInputResponses() / RequestState() accessors. Client API: client.CallToolWithInputs(ctx, c, name, args, handler, opts...) auto-loops; client.DefaultInputHandler(c) routes inputRequests through Client.HandleServerRequestWithContext (single source of truth — same dispatcher real server-initiated requests use). 7 conformance scenarios in conformance/mrtr/ + 1 deferred skip (MRTR→Tasks composition; tracked in matching server/mrtr_test.go skip).
- mcp-list-ttl: SEP-2549 cache-freshness hint on every paginated list response (tools/list, prompts/list, resources/list, resources/templates/list). `TTL *int` (seconds) on all four core list result types — pointer semantics preserve the spec's three states: nil = absent (no guidance), &0 = "do not cache", &N>0 = fresh for N seconds. server.WithListTTL(seconds) Option (negative = unset, ≥0 = emitted) — uniform across endpoints. Client-side: new ListXPage(cursor) typed helpers (ListToolsPage / ListPromptsPage / ListResourcesPage / ListResourceTemplatesPage) return the full result envelope so callers can read TTL alongside NextCursor; existing zero-arg ListTools() etc. + Tools(ctx) iterator unchanged for backward compat. 5 conformance scenarios in conformance/list-ttl/ across 3 server processes (positive / zero / unset).
- mcp-file-inputs-experimental: SEP-2356 file inputs — partial implementation (Phase 1.1 + 1.2 + early 1.7). core.FileInputDescriptor{Accept, MaxSize *int} + core.FileInputProperty / FileInputArrayProperty / ExtractFileInputDescriptor schema helpers (x-mcp-file JSON Schema extension keyword on string/uri properties). RFC 2397 base64 data URI codec: core.EncodeDataURI(data, mediaType, filename) + core.DecodeDataURI(uri) (returns data, mediaType, filename, err) + core.IsDataURI prefix check. Filenames are percent-encoded via url.PathEscape on the `name=` parameter. Non-base64 data URIs rejected with ErrNonBase64DataURI. ClientCapabilities.FileInputs marker + core.HasFileInputs(ctx) capability check. examples/file-inputs/ demokit walkthrough with three tools (upload_image, analyze_documents, process_any_file) and embedded testdata/ fixtures (PNG, PDFs, text). Phases 1.4 (server validation), 1.5 (capability gating in tools/list), 1.6 (client.PrepareFileArg), 1.7 (full conformance) pending; Phase 2.1 (bridge file picker) shipped — see mcp-apps-bridge-selectfile-experimental.
