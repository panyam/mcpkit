...
- mcp-tasks-v2-conformance: conformance/tasks-v2/ — 27 scenarios testing the full SEP-2663 surface. Covers extension advertisement + negotiation gating, polymorphic tools/call, DetailedTask with inlined result/error/inputRequests, ttlSeconds wire field rename, ack-only cancel, tasks/update MRTR loop, Mcp-Name HTTP header (set + absent paths), SEP-2575 per-request opt-in, related-task _meta absence on inlined results, status notifications. Self-contained make target (build + serve + test + teardown). Migration guide: docs/TASKS_V2_MIGRATION.md.
