...
- **`.github/workflows/vulncheck.yml`** scans the published surface weekly. That schedule is the guarantee. It is time-triggered rather than tied to pushes because the advisory database moves independently of this repository: a CVE published after your last commit affects code that has not changed. The workflow also carries a release-tag trigger, which is a second pass rather than the thing being relied on: that leg raised no event at all from v0.4.0 through v0.6.0, so five releases were covered by the weekly run alone and the wording here claimed more than the mechanism delivered (#1412). Confirm it with `make check-release-workflows V=<tag>` after tagging, per `RELEASING.md`.
