...
- **MCP Events conformance, phases 2-4** (issue 1374). Phase 1 merged: `testconf-events` runs the discovery and poll scenarios, 45 of 131 declared requirement rows. Still to come are the push and webhook scenarios, a run against a non-mcpkit implementation, and posting results to `#triggers-events-wg`, which is what the WG commitment actually promised. Blocked on nothing except effort, except that the upstream draft (`modelcontextprotocol/conformance` PR 504) cannot merge until MCP Events gets a SEP number, since the check ids carry a `sep-9999-` placeholder. Our own divergences the suite found are issue 1380.
