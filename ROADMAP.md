...
Stable v0.4.0 shipped 2026-08-03, inside the 30-day window after the 2026-07-28 spec GA. Four releases have followed: v0.5.0 (2026-08-06), v0.5.1 (2026-08-07), v0.5.2 (2026-08-21) and v0.6.0 (2026-09-17). Scope and migrations for each are in `CHANGELOG.md` and `docs/releases/`.

Note that v0.6.0 was the first release whose tag-triggered workflows were expected to run and did not; none of the five had, and nothing noticed. `RELEASING.md` now carries a step that checks. See `CLAUDE.md` § Cross-cutting rules.
...
- **MCP Events conformance, phases 2-4** (issue 1374). Phase 1 merged: `testconf-events` runs the discovery and poll scenarios, 45 of 131 declared requirement rows, at 12/12 and 29/29 once PR 1416 lands and closes issue 1380. Still to come are the push and webhook scenarios, a run against a non-mcpkit implementation, and posting results to `#triggers-events-wg`, which is what the WG commitment actually promised. Blocked on nothing except effort, except that the upstream draft (`modelcontextprotocol/conformance` PR 504) cannot merge until MCP Events gets a SEP number, since the check ids carry a `sep-9999-` placeholder.

  One question goes to the WG with it: the sketch puts the events capability top-level under `capabilities.events`, and the sketch author's own reference server puts it in the SEP-2133 extensions map. mcpkit follows the document.
