# Fine-grained authorization: scope step-up (UC2) and ephemeral credentials (UC3)
...
- **Connect to MCP server with read-only token** — JWT validation against the AS's JWKS endpoint succeeds. The token is valid, just limited in scope.
- **Call read_document, which succeeds (tools-read is sufficient)** — The read_document tool only requires tools-read scope. Our token has it, so the call succeeds.
...
- **Retry update_document with a broader token, which SUCCEEDS** — New session with the broader token. update_document succeeds because the token includes tools-call.
...
- **Retry initiate_payment with a RAR-bound token, which SUCCEEDS** — The server's initiate_payment handler reads authorization_details from the JWT claims and validates that a payment_initiation entry matches the request (amount, currency, payee). It does, because the host minted exactly the token the server asked for in the previous denial.
...
sequenceDiagram
    ...
    Note over Host,AS: Step 4: Call read_document, which succeeds (tools-read is sufficient)
    ...
    Note over Host,AS: Step 7: Retry update_document with a broader token, which SUCCEEDS
    ...
    Note over Host,AS: Step 10: Retry initiate_payment with a RAR-bound token, which SUCCEEDS
...
### UC1 vs UC2/UC3, and when the host reacts
...
JWT validation against the AS's JWKS endpoint succeeds. The token is valid, just limited in scope.
...
### Step 4: Call read_document, which succeeds (tools-read is sufficient)
...
### Step 7: Retry update_document with a broader token, which SUCCEEDS
...
### Step 10: Retry initiate_payment with a RAR-bound token, which SUCCEEDS

The server's initiate_payment handler reads authorization_details from the JWT claims and validates that a payment_initiation entry matches the request (amount, currency, payee). It does, because the host minted exactly the token the server asked for in the previous denial.
