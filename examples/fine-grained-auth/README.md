...
- **UC2, scope step-up.** The server returns `HTTP 403 + WWW-Authenticate: Bearer scope="..."` when a tool requires a scope the bearer token lacks; the host parses the required scopes per RFC 6750 and re-authorizes with the union.
- **UC3, ephemeral credentials (RFC 9396 RAR).** The server returns a JSON-RPC error carrying a `remediationHint` of type `oauth_authorization_details`; the host uses those details *verbatim* in a token request to mint a transaction-bound credential.
...
- The custom `isError=true` and `tool=` color rules on the demo server logger are passed as variadic extras to `common.NewMCPLogger`, so they layer on top of the canonical 5-rule set without re-declaring it.
