...
docker compose start event-server-1`).Default(),
	...
	demo.Step("Walkthrough — fire events/list × N, log X-Replica rotation.").
		...
		VerbatimVariants("Same events/list call by curl — watch X-Replica rotate",
...
docker compose up -d`).Default(),
	...
	// Three knobs orthogonal on the same `make webhook` target —
	// TTL_MS shapes what we ask for / what we get; REPLY_STATUS shapes
	// how the server reacts to each delivery; EXIT_AFTER shapes whether
	// the receiver lives.
	// -----------------------------------------------------------------
...
make logs | grep -E "no-expiry subscription dropped|FailingContinuouslySince"`).Default(),
	...
	demo.Section("Phase 7 — Dynamic source topology (PULL pattern)",
