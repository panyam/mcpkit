...
### Just want a server to point a client at?

No checkout, no Go toolchain, no build. One container:

```bash
docker run -p 8080:8080 ghcr.io/panyam/mcpkit-event-server
```

That is a single replica with in-memory stores and an anonymous principal, and
it serves all three delivery modes. Enough to develop an MCP client against.

For the multi-replica shape (nginx round-robin, Postgres-backed stores, Redis
fanout), one file and one command:
...
This is the **same file** `make up` uses. Development layers `compose.dev.yaml`
...
v2.23+ for inline `configs:`. Verify it came up with `make stack-smoke`, which drives a real MCP session
through to `events/poll`. For the dev stack (`make up`) use `make smoke`
instead: that one has auth on, so the check is that anonymous callers are
*refused*.

Both files are generated by `tools/gen-compose` and regenerated by
`make gen-compose`. There used to be a separate hand-maintained
`docker-compose.yaml` for development, and it had already drifted from the
published stack on five environment variables within days. Hence one file.

The interactive subscriber tools (`make webhook`, `make streamer`,
`make poller`) are not in that stack. They run on the host from a checkout, so
the sections below are still how you drive the guided demo.

### Development flow (from a checkout)

One-time setup — install the `oneauth` CLI:

```bash
go install github.com/panyam/oneauth/cmd/oneauth@v0.1.36
```

Bring up the stack:

```bash
just up           # builds from your checkout, N=3 replicas + Keycloak + Postgres + Redis, auth on. Add BUILD=true to force a rebuild.
...
├── events-stack.yaml       # GENERATED — the one canonical stack
├── compose.dev.yaml        # GENERATED — dev overlay (build + DB ports)
