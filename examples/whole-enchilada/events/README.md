...
`events-stack.yaml` is generated by the same `tools/gen-compose` that generates
the development compose below, so the two cannot drift on replica count, env
vars or nginx upstreams. It is regenerated by `make gen-compose`.
