...
if res.IsInputRequired() {
	...
	demo.Step("Auto-loop: CallToolWithInputs runs the round-trip").
		...
		Note("`client.CallToolWithInputs(ctx, c, name, args, handler)` collapses the whole loop. `DefaultInputHandler` synthesizes a server-to-client request for each `inputRequest` and routes it through `client.HandleServerRequestWithContext`, the single source of truth for how the client responds to MCP method requests, whether they arrived over the back-channel or inlined inside an InputRequiredResult.").
		VerbatimVariants("Reproduce on the wire",
...
// res.Sync.Content[0].Text == "Hello, Alice!"`),
	...
	demo.Step("Multi-round: server accumulates answers across rounds via requestState").
		...
		Arrow("Host", "Server", "retry with inputResponses{step2} (NOT step1, which is already in requestState)").
...
// res.Sync.Content[0].Text holds the final greeting.`),
	...
	demo.Step("Tracing: rounds 2+ link back to round 1 (SEP-414 P6)").
		...
		Note("To see the stitched trace, bring up the LGTM observability stack alongside the demo: `(cd ../../docker/observability && docker compose up -d)` then `EXPORTER=auto just serve` here. Open Grafana → Tempo → service `mrtr-demo`. Click any round-2/3 dispatch span's Link entry, and one click navigates to round 1's trace tree, looking at the original input-required handoff that the final ToolResult resolved from.").
		Note("Without this PR the same operation produced N unrelated traces, so operators looking at round N had no way to navigate to round 1. The vendor-namespaced `_meta.io.modelcontextprotocol/tracelink` field is mcpkit-specific today; upstream WG standardization of a bare cross-SDK name is a future-discussion item.")
