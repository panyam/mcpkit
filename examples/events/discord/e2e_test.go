...
func TestE2EStreamDelivery(t *testing.T) {
	...
	case ev := <-got:
		...
		assert.NotNil(t, ev.Cursor, "discord.message is cursored, so the wire MUST carry a non-null cursor")
...
func TestE2EHealthSignalsEndToEnd(t *testing.T) {
	...
	webhooks.Register(events.RegisterParams{CanonicalKey: canonical, DerivedID: "sub_health", URL: webhookReceiver.URL, Secret: "whsec_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", MaxAgeMs: 0})
