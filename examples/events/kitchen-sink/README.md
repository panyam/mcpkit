...
```bash
just serve            # terminal 1 — start the server (port 8080)
just demo             # terminal 2 — interactive walkthrough
just demo-test        # OR: non-interactive (for CI / scripting)
...
EXPORTER=stdout just serve

# Hand spans to the docker/observability LGTM stack.
# Bring it up first: (cd ../../../docker/observability && docker compose up -d)
# Then either:
just serve-otlp                          # equivalent to EXPORTER=auto — silent fallback if stack isn't up
EXPORTER=otlp just serve                 # warns on dial failure
```
