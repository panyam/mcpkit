# MCP Tasks v2 (SEP-2663): server-directed async plus MRTR

Walks through the v2 Tasks extension where the *server* decides whether to create a task, so clients no longer send a task hint. Polymorphic tools/call, inlined results, ack-only cancel, and the new tasks/update flow that closes the elicit/sample (MRTR) loop.
...
### Step 3: slow_compute (no task hint!), so the server creates a task → ToolCall returns the Task variant
