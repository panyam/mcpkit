...
func SetupLogs(ctx context.Context, opts ...SetupOption) (*slog.Logger, ShutdownFunc, error) {
	...
	case ExporterStdout:
		...
		if err != nil {
			return nil, nil, fmt.Errorf("commonotel.SetupLogs stdout exporter: %w", err)
		}
	...
	case ExporterOTLP:
		if err := probeOTLPEndpoint(cfg.otlpEndpoint); err != nil {
			log.Printf("commonotel.SetupLogs: OTLP endpoint %s unreachable (%v) — falling back to slog.Default. Bring up docker/observability/ to enable trace-correlated logs, or set EXPORTER='' to silence this warning.", cfg.otlpEndpoint, err)
			return slog.Default(), noopShutdown, nil
		}
		...
		if err != nil {
			log.Printf("commonotel.SetupLogs: OTLP exporter init failed (%v) — falling back to slog.Default.", err)
			return slog.Default(), noopShutdown, nil
		}
		logger, shutdown := buildLogger(exp, &cfg)
		return logger, shutdown, nil
	...
	case ExporterAuto:
		...
		if err != nil {
			log.Printf("commonotel.SetupLogs (auto): OTLP exporter init failed (%v) — falling back to slog.Default.", err)
			return slog.Default(), noopShutdown, nil
		}
		logger, shutdown := buildLogger(exp, &cfg)
		return logger, shutdown, nil
	...
	default:
		return nil, nil, fmt.Errorf("commonotel.SetupLogs: unknown exporter %q (expected %q, %q, %q, or empty)", cfg.exporter, ExporterStdout, ExporterOTLP, ExporterAuto)
	}
}
