// Drop-in mcpkit equivalent of upstream's basic-server-svelte example.
//
...
// cloned upstream and run `npm run build` for basic-server-svelte.
//
...
func serve() {
	...
	htmlPath := filepath.Join(extAppsDir, "examples", "basic-server-svelte", "dist", "mcp-app.html")
