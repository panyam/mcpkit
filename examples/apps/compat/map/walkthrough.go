...
// a server started by `make serve` in another terminal.
//
// Walks six steps:
//
...
func runDemo() {
	...
	demo.Section("Setup",
		...
		"```",
		"",
		"Any MCP host can connect to the running server (Claude Desktop, VS Code, MCPJam, basic-host). The walkthrough below acts as a scripted host that issues the protocol calls directly through `*mcpkit/client.Client` — no LLM, no browser. The same calls drive the iframe when you run `make demo-app EXAMPLE=map` in basic-host (see the [centralized guide](../README.md#other-ways-to-test-a-fixture)).",
	)
