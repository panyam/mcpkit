...
func runDemo() {
	...
	demo := demokit.New("wiki-explorer — Wikipedia link graph over MCP Apps").
		Dir("wiki-explorer").
		Description("Walks the get-first-degree-links round trip end-to-end as a scripted MCP client: initialize, tools/list, three tools/call shapes (default URL, different article, invalid URL → string error path), and resources/read. The fixture mirrors upstream's wiki-explorer-server example: server-side Wikipedia HTML fetch + regex link extraction (~380 outbound links for the default MCP page), nullable `error` string in the result, force-directed graph rendered iframe-side. Run `make serve` in another terminal first.").
		Actors(
	...
	demo.Section("Setup",
		...
		"```",
		"",
		"Any MCP host can connect to the running server (Claude Desktop, VS Code, MCPJam, basic-host). The walkthrough below acts as a scripted host that issues the protocol calls directly through `*mcpkit/client.Client` — no LLM, no browser. The same calls drive the iframe when you run `make demo-app EXAMPLE=wiki-explorer` in basic-host (see the [centralized guide](../README.md#other-ways-to-test-a-fixture)).",
	)
