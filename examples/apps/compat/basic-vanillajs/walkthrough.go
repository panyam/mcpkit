...
// runDemo is the basic-vanillajs walkthrough. Acts as a scripted MCP host
...
func runDemo() {
	...
	demo := demokit.New("basic-vanillajs — minimum-viable MCP App").
		Dir("basic-vanillajs").
		Description("Walks the get-time round trip end-to-end as a scripted MCP client: initialize, tools/list, tools/call, and resources/read on the App's iframe HTML. The fixture mirrors upstream's basic-server-vanillajs example byte-for-byte at the protocol surface, so the trace here is also a faithful upstream-parity check. Run `just serve` in another terminal first.").
		Actors(
	...
	demo.Section("Setup",
		...
		"```",
		"",
		"Any MCP host can connect to the running server (Claude Desktop, VS Code, MCPJam, basic-host). The walkthrough below acts as a scripted host that issues the protocol calls directly through `*mcpkit/client.Client` — no LLM, no browser. The same calls drive the iframe when you run `just demo-app EXAMPLE=basic-vanillajs` in basic-host (see the [centralized guide](../README.md#other-ways-to-test-a-fixture)).",
	)
...
curl -s -X POST `+serverURL+`/mcp \
  ...
  -d '{"jsonrpc":"2.0","method":"notifications/initialized"}'`,
    ...
    core.ClientInfo{Name: "basic-vanillajs-host", Version: "1.0"},
)
