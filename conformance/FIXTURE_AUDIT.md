...
Snapshot of every fixture / driver / script behind mcpkit's conformance program, graded against the API paths a documented mcpkit user would reach for. Companion artifact to [`UPSTREAM_AUDIT.md`](UPSTREAM_AUDIT.md): that file grades mcpkit against the upstream test set, this file grades mcpkit against itself, asking whether the surfaces we use to *pass* those tests are the same surfaces we *recommend* to users.
...
Tool registrations use `core.TextTool` / `core.TypedTool + core.WithToolMeta(&core.ToolMeta{UI: ...})`, which is canonical. Two deliberate deviations, both documented inline:
