...
func (a *App) onTaskDetach(bt *client.BackgroundTask) {
	...
	a.tasksMu.Unlock()
	a.ui.Emit(UIEvent{Kind: UITaskDetached, Task: bt})
}
...
func (a *App) onTaskComplete(serverID string, bt *client.BackgroundTask) {
	...
	a.tasksMu.Unlock()
	a.ui.Emit(UIEvent{Kind: UITaskCompleted, Task: bt})
...
func (a *App) cancelTask(id string) {
	...
	a.tasksMu.Unlock()
	if bt == nil {
		a.ui.Emit(UIEvent{Kind: UIMessage, Message: fmt.Sprintf("no running task %q (see /tasks)", id)})
		return
	}
