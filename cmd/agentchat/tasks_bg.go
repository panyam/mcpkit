...
// the background.
func (a *App) onTaskDetach(bt *agent.BackgroundTask) {
	a.tasksMu.Lock()
...
func (a *App) onTaskComplete(serverID string, bt *agent.BackgroundTask) {
	a.tasksMu.Lock()
...
func (a *App) snapshotTasks() []*agent.BackgroundTask {
	a.tasksMu.Lock()
	defer a.tasksMu.Unlock()
	out := make([]*agent.BackgroundTask, 0, len(a.bgTasks))
