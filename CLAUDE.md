...
- **apps/compat Playwright baselines are platform-pinned** (PR 534): `make test-apps-playwright` runs upstream's `ext-apps` Playwright suite against a mcpkit-Go drop-in under `examples/apps/compat/<name>/`. Per-fixture baselines live under `__snapshots__/` in the fixture's own dir (not in upstream's tree). Chromium font fallback differs between Linux Docker and macOS, so regenerate baselines (`UPDATE_SNAPSHOTS=1 make test-apps-playwright`) on the same OS that comparison runs use; otherwise visual checks fail with structural ~5–10px layout shifts that look like real regressions. Wrapper env vars (`HARNESS_PORT` / `SANDBOX_PORT` / `FIXTURE_PORT` / `EXT_APPS_DIR`) and the drop-in pattern: see `examples/apps/compat/README.md`. Docker mode + platform-mismatch warning tracked as 535 / 536.
