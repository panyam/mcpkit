...
- **SEP-414 server-side trace propagation (PR 644 contract + PR 649 wiring)**: `server.WithTracerProvider(nil)` and `WithTracerProvider(core.NoopTracerProvider{})` both skip the trace middleware install entirely — zero overhead on the default path. When a non-Noop provider is configured the trace middleware sits OUTERMOST, so user `WithMiddleware` runs inside the span (recorded latency includes user mw). Inbound trace context precedence: in-band `params._meta.traceparent` wins over the `core.WithTraceContext` value on ctx (populated by the SEP-2028 HTTP `traceparent` / `tracestate` header bridge in `streamable_transport.handlePost`). Outbound `_meta.traceparent` injection wraps `sc.notify` / `sc.request` via `core.WrapSessionNotifyFunc` / `core.WrapSessionRequestFunc` — sits OUTSIDE any user-registered `NotifyInterceptor` / `RequestInterceptor`, so user interceptors observe the augmented wire form (intentional, useful for audit/observability). Explicit handler-set `_meta.traceparent` on outbound messages is preserved (not clobbered). Malformed inbound traceparent drops to zero (W3C "MUST NOT forward"); dispatch still succeeds, span emits with no parent. `_meta` trace keys are bare W3C names (`traceparent` / `tracestate`), NOT under `io.modelcontextprotocol/` — that namespace is reserved for MCP-defined fields. P3 client-side spans, P4 `ext/otel/` adapter, P5 examples still TODO. See `docs/SEP_414_OTEL.md`.
