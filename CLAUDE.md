...
make testconf-upstream-audit # Audit mcpkit against modelcontextprotocol/conformance@main (informational, not gated) → conformance/UPSTREAM_AUDIT.md
...
- **Upstream conformance audit (`conformance/UPSTREAM_AUDIT.md`)**: regenerated by `make testconf-upstream-audit`. Informational snapshot of mcpkit graded against every scenario upstream currently ships, grouped by SEP. Not a CI gate (excluded from `testall`). Use the report to prioritize implementation work; each `fail` / `harness-gap` row is a follow-up. The `also-covered-by-fork` tag is hand-maintained in `scripts/conformance-audit-report.ts` — update the `FORK_OVERLAP` map when SEP-fork targets gain or lose coverage.
