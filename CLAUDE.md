...
- **apps/compat Playwright baselines are platform-pinned** (PR 534 + 537): `make test-apps-playwright` runs upstream's `ext-apps` Playwright suite against a mcpkit-Go drop-in under `examples/apps/compat/<name>/`. Per-fixture baselines live under `__snapshots__/` in the fixture's own dir; filenames carry a `{platform}` suffix (`-darwin` / `-linux`) so both coexist. Linux is canonical, generated under `make test-apps-playwright-docker` (`DOCKER=1`, runs inside `mcr.microsoft.com/playwright:v1.57.0-noble` — same image upstream uses for `test:e2e:docker`, so the linux PNG is byte-identical to upstream's CI). Native mode (default) compares against the host OS's baseline for fast local iteration; the wrapper warns at startup if the current platform has no committed baseline. Wrapper env vars (`HARNESS_PORT` / `SANDBOX_PORT` / `FIXTURE_PORT` / `EXT_APPS_DIR` / `DOCKER`) and the drop-in pattern: see `examples/apps/compat/README.md`.
