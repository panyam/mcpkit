...
**Durable semantic store (issue 1019) shipped:** `gormstore.SemanticMemoryStore` (`experimental/agent/store/gorm`) is the durable, ANN-backed counterpart to `InMemorySemanticStore` — Postgres + pgvector, same `MemoryStore` contract, same client-side `Embedder`, but the top-k lives in SQL (`ORDER BY embedding <=> $query LIMIT k` over an HNSW cosine index) so recall survives process exit and scales past a scratchpad. A namespace column scopes many sessions to one table; the per-request `Namespace` field stays with issue 1003. agentchat routes a postgres `--session-store` + `--memory-embed-model` here (one spec backs runs, blobs, and semantic memory).

**Session-scoping + durable backends (issue 1003) shipped:** `PutMemoryRequest` / `ListMemoriesRequest` / `DeleteMemoryRequest` gained a per-request **`Namespace`** — one independent scratchpad per session/user (empty = the shared default), and recall never crosses namespaces. Every backend honors it: the two in-memory stores partition their maps by namespace; `gormstore.SemanticMemoryStore` reads `req.Namespace` (falling back to its instance-level `WithMemoryNamespace`, so one table serves many sessions or one store binds to one); and a new plain durable backend, **`redisstore.MemoryStore`** (`experimental/agent/store/redis`), stores each namespace as one Redis hash (`<prefix>:<namespace>` → key→JSON), substring recall, optional per-namespace TTL for GC'ing abandoned sessions. `MemorySource` threads a namespace via `WithMemoryNamespaceFunc(func() string)` (nil = the shared scratchpad, unchanged).

**Admin / inspection is per-store, and free where the substrate is queryable.** The pgvector store is a plain Postgres table — inspect it in DBeaver directly: `SELECT namespace, key, body, created_at FROM agent_memories ORDER BY created_at` (the `body` jsonb holds the `MemoryItem`; the vector column is not meant for eyeballing). The redis store is `HGETALL mcpkit.agent.memory:<namespace>`. The in-memory store has no inspection surface by nature. A uniform admin UI over the seam is a later, per-store concern, not part of the storage backends.

**agentchat session-scoping (issue 1140) shipped:** `MemoryConfig.SessionScoped` (opt-in) makes the host pass `WithMemoryNamespaceFunc(a.currentRunID)`, so each RunStore run gets its own scratchpad (`--memory-session-scoped`; default off, a no-op + warning without a `--session-store`). The lock-safe accessor the 1003 note wanted is `currentRunID`, backed by an `atomic.Value` mirror of `runID` written through a single `setRunID` — `App.RunID` takes `turnMu`, which `RunTurn` holds while the namespace func runs (both the memory tools and the summary/recall injection), so a `turnMu` read there would deadlock.

**Sub-agents and memory (issue 1151):** injection over shared store. A sub-agent has no working memory by default, and if it needs one the child Runner owns it entirely (its store/namespace, opaque to the parent — the encapsulation of a stateful MCP tool's DB), never a namespace into the parent's store. A child's location is not guaranteed, so shared parent memory assumes a co-location that A2 wire-serializability forbids; parent→child is params + injection. Hierarchy (parent recall across children) waits on a prefix/hierarchical namespace query the seam lacks (exact-match today).

**Deferred:** a Redis **semantic** (vector) backend is a follow-up; durable semantic recall is the pgvector store's job today.
