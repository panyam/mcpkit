...
| **Coding surface (sandboxing, hooks, repo map, LSP)** | 🟡 **decided** — #1059 closed | Hooks landed in `experimental/agent/` as `ToolMiddleware` (#1248). The rest are an **extension** over the general harness, not a separate module: epic **#1252**. First child shipped: file checkpoint / rewind **✅ #1267** in `experimental/agent/ext/checkpoint`, the first real `host.Extension` — building it found the contract had no lifecycle hook, so `TurnStart` was added. Anchored edit primitive **✅ #1275** in `experimental/agent/ext/files`: content anchors locate a hunk, a content hash detects the file changing underneath, and neither substitutes for the other. It landed as `files` rather than `coding` because nothing in it is code-aware, which splits the epic's children along a line worth keeping. Toolset completed **✅ #1284** — `write_file` (no create-or-overwrite mode; `expect_hash` alone separates create from replace), plus `list_files` / `search_files`, so an agent can find what it edits | LSP-in-loop **✅ #1301** landed as `experimental/agent/ext/lsp`, named after the mechanism rather than the use case, on the same reasoning that made #1275 `files` rather than `coding`. It is the first `ContextStages` consumer and the first extension to own a subprocess, so it added `host.Extension.Close`. Building it found that **`ContextStage` is per-turn while an edit-check-fix loop is per-step**, which is why diagnostics ship on two paths; that plus three smaller seam gaps are recorded on #1240. Repo map **✅ #1311** could not be its own module: C4 blocks `ext/repomap` importing `ext/lsp`, and working out why showed the two would run two language servers over the same tree, so the map is a second contribution from `ext/lsp` gated by `Config.RepoMap`. Command execution and its sandbox **✅ #1312** landed as `experimental/agent/ext/exec`, reordered ahead of the test loop because the epic shipped the dangerous half first. It also corrected the epic twice: sandboxing cannot live in a `ToolSource` wrapper (most tools are in-process functions or MCP calls with no subprocess to confine), and the sandbox is not the primary defense, the approval gate is. Only the darwin `sandbox-exec` backend ships, with Linux carved out to #1320. Remaining #1252 child: test/build loop (#1313), now unblocked. Still **blocked**: diff-level review (#1288). Host decomposition **#1251** runs in parallel |
