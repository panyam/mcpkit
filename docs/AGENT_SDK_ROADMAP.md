...
| **Host surface** | ✅ #984–#992 | slash-command registry, `ConnectionRegistry` + runtime `/provider`, `HostEvent`/`Observer` render seam, notebook renderer (#1001), interactive `/mcp` + `/sessions` overlay (#1095, `focusLayer`/`modalHost` seam + `client.Group.Reconnect`), per-server tool view (#1117) + oauth login action / authorization-code auth type (#1116, #907), bubbletea TUI, playground | context-assembly pipeline **⏳ #1024,#1026**; full focus-stack (base+overlay as peer layers) **⏳ #1063 C4** |
