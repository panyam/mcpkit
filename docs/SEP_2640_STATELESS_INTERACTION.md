...
- 2026-06-04 (stateless subscription caps): mcpkit ships `WithStatelessSubscriptionCap` / `WithStatelessSubscriptionRateLimit` / `WithStatelessSubscriptionScope` (PR 599). The wire surface guarded (`subscriptions/listen`) is in the 2026-07-28 RC, not Final; the cap and rate-limit mechanism is mcpkit-internal and not spec-defined. Outstanding action: if SEP-2575 reaches Final with a normative subscription-cap mechanism, revisit `server/streamable_subscriptions.go` to align so we don't ship two competing semantics.
