...
func TestExtractMcpHeaderParams_NilSchema(t *testing.T) {
	got := ExtractMcpHeaderParams(nil)
	if len(got) != 0 {
		t.Errorf("nil schema should yield empty map, got %v", got)
	}
}

func TestExtractMcpHeaderParams_NoAnnotations(t *testing.T) {
	schema := map[string]any{
		"type":       "object",
		"properties": map[string]any{"name": map[string]any{"type": "string"}},
	}
	got := ExtractMcpHeaderParams(schema)
	if len(got) != 0 {
		t.Errorf("schema without annotations should yield empty map, got %v", got)
	}
}

func TestExtractMcpHeaderParams_PrimitiveTypesOnly(t *testing.T) {
	schema := map[string]any{
		"type": "object",
...
func TestEncodeMcpHeaderValue_NilOmitsHeader(t *testing.T) {
	if _, ok := EncodeMcpHeaderValue(nil); ok {
		t.Error("nil value should report ok=false (omit header)")
	}
}

func TestEncodeMcpHeaderValue_PlainASCII(t *testing.T) {
...
func TestEncodeMcpHeaderValue_RequiresBase64(t *testing.T) {
	for _, tc := range []struct {
		name, in, want string
	}{
...
func TestEncodeMcpHeaderValue_Numbers(t *testing.T) {
	for _, tc := range []struct {
		name string
		in   any
		want string
...
func TestEncodeMcpHeaderValue_Booleans(t *testing.T) {
	if got, _ := EncodeMcpHeaderValue(true); got != "true" {
		t.Errorf("true → %q, want %q", got, "true")
	}
	if got, _ := EncodeMcpHeaderValue(false); got != "false" {
...
func TestValidateMcpHeaderSchema_Valid(t *testing.T) {
	schema := map[string]any{
		"type": "object",
		"properties": map[string]any{
...
func TestValidateMcpHeaderSchema_NilSchemaPasses(t *testing.T) {
	if err := ValidateMcpHeaderSchema(nil); err != nil {
...
func TestValidateMcpHeaderSchema_InvalidCases(t *testing.T) {
	for _, tc := range []struct {
		name   string
		schema map[string]any
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			if err := ValidateMcpHeaderSchema(tc.schema); err == nil {
