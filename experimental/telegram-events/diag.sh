...
# rpc: POST a JSON-RPC request, extract the data: line from the SSE response
rpc() {
  local sid="$1" body="$2"
  curl -s -N -X POST "$MCP" \
...
echo "=== Telegram Events — Diagnostic ==="
echo ""

# 1. Initialize — session ID is in response header
echo "--- initialize ---"
...
# 2. Send notifications/initialized (required before other methods work)
...
# 5. events/list (protocol method)
echo "--- events/list ---"
rpc "$SID" '{"jsonrpc":"2.0","id":4,"method":"events/list","params":{}}' \
  | jq '.result.events[]'
echo ""

# 6. events/poll (protocol method, cursor=0)
echo "--- events/poll (cursor=0) ---"
rpc "$SID" '{"jsonrpc":"2.0","id":5,"method":"events/poll","params":{"subscriptions":[{"id":"diag","name":"telegram.message","cursor":"0"}]}}' \
  | jq '.result.results[0]'
echo ""

# 7. telegram://messages/recent resource
echo "--- telegram://messages/recent ---"
RECENT=$(rpc "$SID" '{"jsonrpc":"2.0","id":6,"method":"resources/read","params":{"uri":"telegram://messages/recent"}}' \
