...
| MRTR (SEP-2322) | [mrtr.md](./mrtr.md) | root | request-anatomy, extension-mechanisms | Multi Round-Trip Requests; `tools/call` returning `IncompleteResult` with `inputRequests` + signed `requestState`; client retries with `inputResponses` + echoed token; stateless across rounds; default `InputHandler` bridges to sampling/elicitation/roots; tasks v2 reuses the same Incomplete shape |
