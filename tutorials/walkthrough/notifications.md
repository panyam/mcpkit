...
| **Task status** *(extension)* | `notifications/tasks/status` | server → client | tasks extension declared at session **or** per-request (SEP-2575) | task lifecycle transition; carries `DetailedTask` + `requestState` ([Q7](#q7-how-do-tasks-fit-in-notificationstasksstatus-vs-subscriptionlisten-patterns)) |
...
**Step 2. Server emits the notification on the standing GET.** No payload beyond the method name, because the notification is a hint, not a diff:
...
- A receives the response after sending the cancel. A's pending table still has the entry, or doesn't, depending on whether A removed it on send-cancel (mcpkit removes it).
- A processes the late response normally. The local caller may already have moved on (timeout, abort), which is a layer above the protocol.
...
- For stdio, "everything on the same pipe", so there's no choice to make.
...
The deeper integration story, how tasks layer their own `notifications/tasks/status` channel *on top of* progress, is in [Q7](#q7-how-do-tasks-fit-in-notificationstasksstatus-vs-subscriptionlisten-patterns).
...
> [!NOTE]
> A server that emits notifications it didn't gate-on is a bug, but it's the server's bug, and the client doesn't pay for it. This asymmetry is what lets the protocol evolve without coordinated upgrades.
...
**Wire shape, a request with a per-request capability override:**
...
SEP-2575 directly affects **request-bound notifications**, the ones emitted *during* the lifecycle of a specific request the client just sent:
...
For SEPs introducing unsolicited notification types, per-request override doesn't apply, and the gate stays session-level via `capabilities.extensions`.
...
**Wire shape, a task status notification:**
...
Tasks don't replace progress. A long-running task can still emit `notifications/progress` (Q4) during execution, the fine-grained "I'm 30% through" updates. The `progressToken` pairing carries through: the task-handler's progress emission reuses the same `_meta.progressToken` mechanism the client opted into on the original `tools/call`.
...
- You know SEP-2575 affects **request-bound** notifications (progress, task status, side-effect emissions from a handler) but **not** unsolicited server-initiated ones (list_changed, resource updates, logging), which stay session-level.
- You know **list_changed is a hint, not a diff**, and the client refetches when it cares.
...
- You know **unknown / un-gated notifications get dropped silently** by the receiver. The asymmetry against unknown requests is what enables forward-compatibility.
