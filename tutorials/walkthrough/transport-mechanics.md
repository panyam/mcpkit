...
- One **HTTP request** carries either a single JSON body (one message, or a batch, meaning an array of messages) **or** a long-lived **SSE event stream** of many events arriving over time (when a POST response upgrades to SSE, or for the standing GET). The stream stays open inside the same one HTTP request; the server emits events as needed; no "new HTTP request" opens for each event.
...
**Setup, or what brought the session to this moment.** Earlier in the chat (could have been seconds or hours ago):
...
**The live moment, where HTTP request #5 starts.** Client → Server, one HTTP request (POST), one JSON-RPC message in the body:
...
**Server → Client, on the same HTTP request.** The response upgrades to SSE; the handler emits two progress notifications, then the final response:
...
- **JSON-RPC ids restart per direction, not per HTTP request.** The client used `id=1`, `id=2` during setup, then jumped to `id=7` for the tool call (other client-originated work in the gap, or just non-monotonic allocation; the client picks, and gaps are fine). Notifications carry no `id`.
...
> [!IMPORTANT]
> `Accept: application/json, text/event-stream` is required, and **both** types must be listed. This is the client telling the server "I can take either type of response." Servers may reject POSTs that omit one.

**One POST per client-initiated operation.** A tool call is one POST containing one JSON-RPC request. Same for `prompts/get`, `resources/read` and the rest: one logical operation, one POST, one request body. JSON-RPC batches (an array of messages in one body) are allowed by the spec but uncommon in practice; a typical mcpkit client/server doesn't use them. The session is reused across many such POSTs over its lifetime, but each POST is its own HTTP transaction.
...
1. Client POSTs `initialize` (no `Mcp-Session-Id` yet, because there isn't one).
...
Servers MAY operate stateless (no session id at all), though many won't. Where there's no session id, there's no standing GET back-channel either; server-initiated traffic can only flow during a per-call SSE upgrade.
...
>
> **Multiple GETs on the same session?** The spec allows it but requires the server to deliver each message on **exactly one** stream, with no fan-out and no duplication. Most implementations (mcpkit included) expect a single standing GET per session and treat extras as redundant.
>
...
HTTP is request/response by nature; full-duplex isn't free. Three pieces, POST, GET and `Mcp-Session-Id`, work together to recover what stdio gets for free from a bidirectional pipe.
...
The **JSON-RPC payload format is transport-agnostic.** Both stdio and streamable HTTP carry the same JSON-RPC payloads and differ only on framing and on how the receiver gets bytes off the channel. JSON-RPC could in principle be replaced (Connect, gRPC, MessagePack, CBOR, …) without changing the MCP semantics on top, but that would be a *spec-level* change, not a transport choice. As of the [2025-06-18 spec](https://modelcontextprotocol.io/specification/2025-06-18), JSON-RPC 2.0 is normative.

> [!NOTE]
> gRPC specifically would be an awkward fit, with a different model (strongly-typed services, streaming as first-class, no real "notification"). Connect (Buf's HTTP/JSON-friendly cousin) maps more naturally. None are on the spec roadmap today.
...
- You know how server→client messages reach the client on each transport: always-open pipe (stdio), per-call SSE upgrade (HTTP POST), or standing GET SSE (HTTP back-channel, independent of any POST, opened proactively after `initialize`, and free to sit idle).
...
- You know IDs are per-direction and that the pending-request table is what makes correlation work, flat and with no parent links.
